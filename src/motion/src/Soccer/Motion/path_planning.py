...
ballRadius = 0.15           # Radius to walk around the ball (in m).
...
class Glob:
    def __init__(self):
        self.pf_coord = [0.5, 0.5 , -math.pi * 3/4]
        self.ball_coord = [0, 0]
        self.obstacles = [[0, 0, 0.15], [0.4, 0.025, 0.2], [0.725, -0.475, 0.2]]
        #self.ball_coord = [2, 0]
        #self.obstacles = [[0.4, 0.025, 0.2], [0.725, -0.475, 0.2], [0.8, 0.55, 0.2], [1.175, 0, 0.2], [1.625, -0.4, 0.2], [1.7, 0.425, 0.2]]
        self.landmarks = {"unsorted_posts": [[ 1.8, 0.6 ],[ 1.8, -0.6 ],[ -1.8, 0.6 ],[ -1.8, -0.6 ]],
...
class PathPlan:
    ...
    def intersection_line_segment_and_circle(self, x1, y1, x2, y2, xc, yc, R):
        ...
        if successCode:
            if 0 < round(t1, 4) < 1 or 0 < round(t2, 4) < 1:
               return True
        return False
    ...
    def path_calc_optimum(self, start_coord, target_coord):
        ...
        #start_yaw = self.coord2yaw(x1 - cx, y1 - cy)
        #dest_yaw = self.coord2yaw(x2 - cx, y2 - cy)
    ...
    def path_calc(self, start_coord, target_coord):
        print(start_coord, target_coord)
            ...
            for j in range(10):
                ...
                if successCode:
                    ...
                    if successCode:
                        ...
                        j = self.check_Obstacle(xp1, yp1, xp2, yp2)
                        if j >= 0 :
                            ...
                            for variant in range(2):
                                if variant == 0:
                                    ...
                                    successCode1, xp1, yp1 = self.external_tangent_line(True,
                                           R1, roundAboutRadius, x1, y1, xc1, yc1, self.glob.obstacles[j][0], self.glob.obstacles[j][1], CW1)
                                    successCode2, xp2, yp2 = self.external_tangent_line(False,
                                           roundAboutRadius, R1, x2, y2, self.glob.obstacles[j][0], self.glob.obstacles[j][1], xc1, yc1, CW)
                                    successCode3, xp3, yp3 = self.external_tangent_line(True,
                                           roundAboutRadius, R2, x1, y1, self.glob.obstacles[j][0], self.glob.obstacles[j][1], xc2, yc2, CW)
                                    successCode4, xp4, yp4 = self.external_tangent_line(False,
                                           R2, roundAboutRadius, x2, y2, xc2, yc2, self.glob.obstacles[j][0], self.glob.obstacles[j][1], CW2)
                                ...
                                if variant == 1:
                                    ...
                                    successCode6, xp2, yp2 = self.internal_tangent_line(False,
                                           roundAboutRadius, R1, xp2, yp2, self.glob.obstacles[j][0], self.glob.obstacles[j][1], xc1, yc1, CW)
                                    successCode7, xp3, yp3 = self.internal_tangent_line(True,
                                           roundAboutRadius, R2, xp2, yp2, self.glob.obstacles[j][0], self.glob.obstacles[j][1], xc2, yc2, CW)
                                ...
                                centers = [[x1, y1, xp1, yp1, xc1, yc1, R1, CW1],
                                            [xp2, yp2, xp3, yp3, self.glob.obstacles[j][0], self.glob.obstacles[j][1], roundAboutRadius, CW],
                                            [xp4, yp4, x2, y2, xc2, yc2, R2, CW2]]
                                price = self.check_Price(x1, y1, x2, y2, xp1, yp1, xp2, yp2, xc1, yc1, CW1, xc2, yc2, CW2, dest)
    ...
    def check_Price(self, x1, y1, x2, y2, xp1, yp1, xp2, yp2, xc1, yc1, CW1, xc2, yc2, CW2, dest):
        price = 0
        for i in range(0, len(dest), 2):
            if self.intersection_line_segment_and_circle(dest[i][0], dest[i][1], dest[i + 1][0], dest[i + 1][1], 
                                                         self.glob.ball_coord[0], self.glob.ball_coord[1],
        ...
        if self.intersection_circle_segment_and_circle(dest[ind][0], dest[ind][1], x2, y2, xc2, yc2, CW2,
                                                    self.glob.ball_coord[0], self.glob.ball_coord[1],
                                                    ballRadius + roundAboutRadiusIncrement): price += 0
        for j in range(len(self.glob.obstacles)):
            roundAboutRadius = self.glob.obstacles[j][2] / 2 + roundAboutRadiusIncrement
            if self.intersection_circle_segment_and_circle(x1, y1, dest[0][0], dest[0][1], xc1, yc1, CW1,
...
if __name__ == '__main__':
    class Example(wx.Frame):
        ...
        def OnPaint(self, e):
            ...
            for obstacle in self.glob.obstacles[1:]:
            ...
            target_x = self.glob.ball_coord[0] - 0.15 * math.cos(target_yaw)
            target_y = self.glob.ball_coord[1] - 0.15 * math.sin(target_yaw)
            target_coord = [target_x, target_y, target_yaw]
            #print(d[2])
            dest, centers, number_Of_Cycles = self.p.path_calc(self.glob.pf_coord, target_coord)
